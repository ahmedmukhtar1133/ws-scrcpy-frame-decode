...
export abstract class BasePlayer extends TypedEmitter<PlayerEvents> {
  ...
  constructor(
    ...
    this.touchableCanvas.id = 'canvas';
    this.touchableCanvas.setAttribute(
      'style',
  ...
  public static getFromStorageCompat(
    ...
    const isDefaultDisplay =
      displayInfo.displayId === DisplayInfo.DEFAULT_DISPLAY;
  ...
  public static getFitToScreenFromStorage(
    ...
    const key = `${this.getFullStorageKey(
      storageKeyPrefix,
      udid,
      displayInfo
    )}:fit`;
  ...
  public static getVideoSettingFromStorage(
    ...
    const saved = this.getFromStorageCompat(
      storageKeyPrefix,
      udid,
      displayInfo
    );
    ...
    return new VideoSettings({
      ...
      crop: crop
        ? new Rect(crop.left, crop.top, crop.right, crop.bottom)
        : preferred.crop,
      ...
      iFrameInterval: !isNaN(iFrameInterval)
        ? iFrameInterval
        : preferred.iFrameInterval,
      sendFrameMeta:
        typeof sendFrameMeta === 'boolean'
          ? sendFrameMeta
          : preferred.sendFrameMeta,
  ...
  public play(): void {
    ...
    //   return
    // }
    console.log('InsidePlayStateSet...');
  ...
  public pushFrame(frame: Uint8Array): void {
    if (!this.receivedFirstFrame) {
      ...
      if (typeof this.qualityAnimationId !== 'number') {
        this.qualityAnimationId = requestAnimationFrame(
  ...
  private updateQualityStats = (): void => {
    ...
    if (this.totalStats.timestamp < oneSecondBefore) {
      this.totalStats = {
        timestamp: now,
        decodedFrames:
          this.totalStats.decodedFrames +
          this.momentumQualityStats.decodedFrames,
        droppedFrames:
          this.totalStats.droppedFrames +
          this.momentumQualityStats.droppedFrames,
        inputFrames:
          this.totalStats.inputFrames + this.momentumQualityStats.inputFrames,
        inputBytes:
  ...
  private drawStats(): void {
    ...
    if (this.perSecondQualityStats && this.momentumQualityStats) {
      const { decodedFrames, droppedFrames, inputBytes, inputFrames } =
        this.momentumQualityStats;
      const { avgDecoded, avgDropped, avgSize, avgInput } =
        this.perSecondQualityStats;
  ...
  public static loadVideoSettings(
    udid: string,
    displayInfo?: DisplayInfo
  ): VideoSettings {
  ...
  public static getFitToScreenStatus(
    udid: string,
    displayInfo?: DisplayInfo
  ): boolean {
    return this.getFitToScreenFromStorage(
      this.storageKeyPrefix,
      udid,
      displayInfo
    );
  ...
  public static saveVideoSettings(
    ...
    this.putVideoSettingsToStorage(
      this.storageKeyPrefix,
      udid,
      videoSettings,
      fitToScreen,
