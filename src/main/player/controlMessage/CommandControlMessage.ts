...
export class CommandControlMessage extends ControlMessage {
  ...
  public static createSetVideoSettingsCommand(
  ...
  public static createSetClipboardCommand(
    ...
    const textBytes: Uint8Array | null = text
      ? Util.stringToUtf8ByteArray(text)
      : null;
  ...
  public static createSetScreenPowerModeCommand(
    mode: boolean
  ): CommandControlMessage {
    const event = new CommandControlMessage(
  ...
  public static createPushFileCommand(
    params: FilePushParams
  ): CommandControlMessage {
    const { id, fileName, fileSize, chunk, state } = params;

    if (state === FilePushState.START) {
      return this.createPushFileStartCommand(
        id,
        fileName as string,
        fileSize as number
      );
    ...
    if (
      state === FilePushState.CANCEL ||
      state === FilePushState.FINISH ||
      state === FilePushState.NEW
    ) {
  ...
  private static createPushFileChunkCommand(
    ...
    const buffer = new Buffer(
      offset + typeField + idField + stateField + chunkLengthField + chunkLength
    );
    buffer.writeUInt8(event.type, offset);
    offset += typeField;
    buffer.writeInt16BE(id, offset);
  ...
  public static pushFileCommandFromBuffer(buffer: Buffer): {
    ...
    if (state === FilePushState.APPEND) {
      ...
      offset += 2;
      fileName = Util.utf8ByteArrayToString(
        buffer.slice(offset, offset + textLength)
      );
    }
