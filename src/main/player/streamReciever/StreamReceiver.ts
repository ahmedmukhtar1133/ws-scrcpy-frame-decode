...
export class StreamReceiver<P extends any> extends ManagerClient<
  any,
...
> {
  ...
  private handleInitialInfo(data: ArrayBuffer): void {
    ...
    let rest: any = new Buffer(new Uint8Array(data, offset));
    ...
    for (let i = 0; i < displaysCount; i++) {
      ...
      if (screenInfoBytesCount) {
        this.screenInfoMap.set(
          displayId,
          ScreenInfo.fromBuffer(rest.slice(0, screenInfoBytesCount))
        );
      ...
      if (videoSettingsBytesCount) {
        this.videoSettingsMap.set(
          displayId,
          VideoSettings.fromBuffer(rest.slice(0, videoSettingsBytesCount))
        );
  ...
  protected onSocketMessage(e: MessageEvent): void {
    ...
    if (e.data instanceof ArrayBuffer) {
      // console.log('SocketVideoMessage', e.data);
      ...
      if (e.data.byteLength > MAGIC_BYTES_INITIAL.length) {
        ...
        if (StreamReceiver.EqualArrays(magicBytes, MAGIC_BYTES_INITIAL)) {
          console.log('IfAboveInsideDeviceMessage...');
        ...
        if (
          ...
          console.log('InsideDeviceMessage', message);
          this.emit('deviceMessage', message);
          return;
        }
      }
      console.log('SocketMessageRecieved', e.data);
      this.emit('video', new Uint8Array(e.data)); // this is necessary for video rendering
    } else {
      console.log('InvalidSocketMessage', e);
    }
  }

  protected onSocketOpen(): void {
    console.log('SocketConnectionOpened...');
  ...
  public sendEvent(event: ControlMessage): void {
    if (this.ws && this.ws.readyState === this.ws.OPEN) {
      console.log('buffer', event.toBuffer());
      this.ws.send(event.toBuffer());
    } else {
      console.log('PusingSendingEventToList');
  ...
  public getDisplayInfo(displayId: number): DisplayInfo | undefined {
    console.log('DisplayMap', this.displayInfoMap);
